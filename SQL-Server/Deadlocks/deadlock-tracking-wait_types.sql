-- pulled from here https://msdn.microsoft.com/en-us/library/ms179984.aspx#WaitTypes but now you can actually know what the crazy wait types mean

DECLARE @locktypes TABLE
(
 lock nvarchar(50),
 details nvarchar(MAX)
);

insert into @locktypes values 
...
('HADR_TDS_LISTENER_SYNC_PROCESSING','Used at the end of an Always On Transact-SQL statement that requires starting and/or stopping anavailability group listener. Since the start/stop operation is done asynchronously, the user thread will block using this wait type until the situation of the listener is known.'),
...
select top 100 l.details, w.* from sys.dm_os_wait_stats w
inner join @locktypes l on w.wait_type = l.lock
